...
do
    echo -e "$tag"
    v=$( [[ $tag =~ ^([^-]*) ]] && echo " --version ${BASH_REMATCH[1]} " || echo "" )
    ...
    docker exec $tag sudo -u apache /bin/bash -c "echo -e \"MAGENTO_EE_PATH=\\n\" >> ~/.m2install.conf && cd /var/www/html && m2install.sh $e$v$s$sd --force && chmod -R 777 ./var ./pub/media ./pub/static ./app/etc && curl http://web2.sparta.corp.magento.com/dev/support/tools/configuration/m2configuration | n98-magerun2 script"
    docker stop $tag
